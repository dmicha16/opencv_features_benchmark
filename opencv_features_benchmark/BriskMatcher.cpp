...
void BriskMatcher::matcher() {

	float match_conf = 0.3f;
	bool try_cuda = false;
	int range_width = -1;

	vector<ImageFeatures> image_features = get_brisk_image_features();
	...
	for (size_t i = 1; i < 4; i++) {
		...
		draw_my_matches(image_features, pairwise_matches, matcher_type, BRISK_R);
